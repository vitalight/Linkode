...
<body><%@ include file="../modules/header.jsp"%>
				...
				<table class="table table-striped table-bordered table-hover"
					...
					<tbody>
						<c:forEach items="${model}" var="postCmt">
							<tr>
								<td>${postCmt.id}</td>
								<td>${postCmt.postid}</td>
								<td>${postCmt.content}</td>
								<td>${postCmt.floor}</td>
								<td><button class="btn btn-default delete" type="button"
									...
									<button class="btn btn-default edit" type="button"
										data-id="${postCmt.id}" data-postid="${postCmt.postid}"
						...
						<div class="col-lg-12">
							<form role="form">
								<div class="form-group">
									<label>postid</label><input class="form-control" type="number"
										step="1" name="postid" />
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									<label>floor</label><input class="form-control" type="number"
										step="1" name="floor" />
								</div>
							</form>
						</div>
	...
	<script
		src="${pageContext.request.contextPath}/static/js/crud/postCmt.js"></script>
