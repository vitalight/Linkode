...
<body><%@ include file="../modules/header.jsp"%>
	<c:set var="type" value="projectApp" /><%@ include
		file="./crud-head.jsp"%>
				...
				<table class="table table-striped table-bordered table-hover"
					...
					<tbody>
						<c:forEach items="${model}" var="projectApp">
							<fmt:formatDate pattern="yyyy-MM-dd" value="${projectApp.time}"
								var="time" />
							<tr>
								<td>${projectApp.id}</td>
								<td>${projectApp.projectId}</td>
								<td>${projectApp.applicantId}</td>
								<td>${projectApp.content}</td>
								<td>${time}</td>
								<td><button class="btn btn-default delete" type="button"
									...
									<button class="btn btn-default edit" type="button"
										...
										data-content="${projectApp.content}" data-time="${time}">
						...
						<div class="col-lg-12">
							<form role="form">
								<div class="form-group">
									<label>projectId</label><input class="form-control"
										type="number" step="1" name="projectid" />
								</div>
								<div class="form-group">
									<label>applicantId</label><input class="form-control"
										type="number" step="1" name="applicantid" />
								</div>
								<div class="form-group">
								...
								<div class="form-group">
									<label>time</label><input class="form-control" type="date"
										name="time" />
								</div>
							</form>
						</div>
	...
	<script
		src="${pageContext.request.contextPath}/static/js/crud/projectApp.js"></script>
